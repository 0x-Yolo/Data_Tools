...
df = pd.read_csv('test_tmp.csv',encoding = 'gbk')
df.to_excel('test_emp.xlsx',index = False)

wdt = df.copy()

df = do.get_data('CreditBondTrading')
# 添加windapi指标
for idx in df.loc[df['时间']=='2021-04-26'].index:
...
# idx: 2964 '价格' 3.1(已撤销)
# idx: 4176 '行权估值''CWSDService: No data.' 日期问题 解决
#  df.loc[4176]

d = pd.read_excel('./examples/21010517-0521.xlsm',sheet_name='数据')
for idx in d.loc[d['到期估值']==0].index:
    code = d.loc[idx,'代码']
    date = d.loc[idx,'时间']
    last_date = d.loc[idx,'估值时间']

    # 检查，存在估值 但仍为0？
    print(w.wsd(code, "yield_cnbd", last_date, last_date,
          "credibility=4;PriceAdj=YTM;TradingCalendar=NIB").Data[0][0],date)
    idx_ = df.loc[(df['代码']==code)&(df['时间']==date)].index 

    net_date = df.loc[idx, '时间']

    df.loc[idx_,'到期估值'] = 0
    df.loc[idx_,'行权估值'] = 0

    df.loc[idx_,'估值偏离'] =( w.wsd(code, "couponrate2").Data[0][0] - df1.loc[idx_,'price'] )* 100
    df.loc[idx_,'剩余期限'] = w.wsd(code,"ptmyear", net_date, net_date, "").Data[0][0]
...
for idx in df.index:
    ...
    if w.wsd(code, "yield_cnbd", last_date, last_date,\
         "credibility=4;PriceAdj=YTM;TradingCalendar=NIB").Data[0][0]!=None:
        continue
    print(idx)
    df.loc[idx,'到期估值'] = 0
    df.loc[idx,'行权估值'] = 0

    df.loc[idx,'估值偏离'] =( w.wsd(code, "couponrate2").Data[0][0] - df.loc[idx,'price'] )* 100
    df.loc[idx,'剩余期限'] = w.wsd(code,"ptmyear", net_date, net_date, "").Data[0][0]
# * 修正0510
for idx in df.index:
    code = df.loc[idx,'代码']
    net_price = df.loc[idx,'价格']
...
def get_stat_table(df):
    ...
    for date in trade_dt_list:
        ...
        stat_table.loc[date, '情绪指数'] = (-1)*df_date.loc[(df_date['风险偏好']>0), '估值偏离'].mean()
...
df = organize(df)
stat = get_stat_table(df)


# * 修正0426的数据 
df_26_30 = do.get_data('CreditBondTrading')
...
l = [(df,'CreditBondTrading_final2',dtypelist)]
for a,b,c in l:
...
df[df['风险偏好']<=0]
stat_table.loc[date, '均价'] = df_date.loc[:, 'price'].mean()
stat_table.loc[date, '笔数'] = df_date.loc[df_date['风险偏好']>0].shape[0]
stat_table.loc[date, '风险偏好指数'] = df_date.loc[:, '风险偏好'.mean()
stat_table.loc[date, '情绪指数'] = -df_date.loc[:,'估值偏离'].mean()
stat_table.loc[date, '平均期限'] = df_date.loc[:, '剩余期限']
stat_table.loc[date, '信用扩张指数'] = df_date.loc[:, '剩余期限'].std()
