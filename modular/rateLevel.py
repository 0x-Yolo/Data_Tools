...
def CurveChange(bond='gz'):
    ...
    ax.legend(ncol=1,loc=3, bbox_to_anchor=(0.15,-0.2),borderaxespad = 0.,fontsize=12,frameon=False)
    ax_.legend([base.date(),end.date()],ncol=1,loc=3, bbox_to_anchor=(0.6,-0.25),borderaxespad = 0.,fontsize=12,frameon=False)
    plt.show()
    return d
...
def msP():
    ...
    df.index = df.date
    df = df['2010':]
    df = [['中票_AAA_1y','中票_AAA_5y','中票_AA+_1y','中票_AA+_5y','date']]
    plt.style.use({'font.size' : 10}) 
    fig,ax = plt.subplots(figsize=(6,4), dpi=300)
