...
def GK():
    ...
    fig,ax = plt.subplots(nrows=n,ncols=1,figsize=(10,4*n),dpi = 300)
    for i in range(n):
        m = maturity[i]
        ...
        ax_.legend(ncol=1,loc=3, bbox_to_anchor=(0.86,-0.16),borderaxespad = 0.) 
        ax[i].legend(ncol=4,loc=3,bbox_to_anchor=(0,-0.16),borderaxespad = 0.)  
    fig.tight_layout()
    return fig
...
def GZ():
    ...
    for i in range(n):
        ...
        ax_ = ax[i].twinx()
        ax_.scatter(temp.index,temp['综收较估值'] ,color='Chocolate',s=10
        )
        ax[i].axhline(y=qcbs_quantile_gk[qcbs_quantile_gk.index ==m].iloc[0][0],\
            ls='--',color='Dodgerblue',label='全场倍数25%',lw=2)
        ax[i].axhline(y=qcbs_quantile_gk[qcbs_quantile_gk.index ==m].iloc[0][1],\
